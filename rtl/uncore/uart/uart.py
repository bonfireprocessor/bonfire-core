    ...
    def register_write():
        if write_cycle:
            if dbus.adr_o[4:2] == 2:
                ...
                fifo_threshold.next = dbus.db_wr[18 + fifo_bits:18]
            elif dbus.adr_o[4:2] == 3:
...
def bonfire_uart(
    ...
    assert dbus.xlen == 32, "bonfire_uart currently supports a 32-bit DBus"
    ...
    fifo_threshold = Signal(modbv(0)[fifo_bits:])
