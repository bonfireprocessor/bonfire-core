...
* Implement RV32I with machine-mode CSR access, precise synchronous traps, and `MRET`.
* Implement `mcycle`/`mcycleh` and `minstret`/`minstreth` counters. Interrupt inputs and arbitration are not implemented yet.
* Pass the RV32I and Zicsr RISC-V compliance suites in MyHDL simulation, including ECALL, EBREAK, and misalignment cases.
