...
class RV32IMDisassembler:
    @classmethod
    def _sign_extend(cls, value, width):
        sign = 1 << (width - 1)
        return (value & (sign - 1)) - (value & sign)

    @classmethod
    def _opcode_name(cls, opcode):
        names = {
    ...
    @classmethod
    def decode_instruction_fields(cls, word):
        ...
        fields = {
            ...
            "opcode7": word & 0x7F,
            "opcode": opcode,
            "opcode_name": cls._opcode_name(opcode),
            "rd": (word >> 7) & 0x1F,
            "funct3": (word >> 12) & 0x7,
            "rs1": (word >> 15) & 0x1F,
            "rs2": (word >> 20) & 0x1F,
            ...
            "imm_i": cls._sign_extend((word >> 20) & 0xFFF, 12),
            "imm_s": cls._sign_extend((((word >> 25) & 0x7F) << 5) | ((word >> 7) & 0x1F), 12),
            "imm_b": cls._sign_extend(
                (((word >> 31) & 0x1) << 12)
                | (((word >> 7) & 0x1) << 11)
                | (((word >> 25) & 0x3F) << 5)
                | (((word >> 8) & 0xF) << 1),
    ...
    @classmethod
    def _disassemble_from_fields(cls, fields):
        ...
        if opcode == op.RV32_SYSTEM:
            ...
            m = names.get(funct3)
            if m is not None:
                src_operand = rs1 if funct3 in (y3.RV32_F3_CSRRW, y3.RV32_F3_CSRRS, y3.RV32_F3_CSRRC) else fields["rs1"]
                return "{} {},0x{:03x},{}".format(m, rd, funct12, src_operand)

        return ".word 0x{:08x}".format(fields["word"])

    @classmethod
    def disassemble_rv32im(cls, word):
        """Disassemble a RV32IM instruction and return text plus decoded fields."""
        fields = cls.decode_instruction_fields(word)
        return cls._disassemble_from_fields(fields), fields

    @classmethod
    def disassemble(cls, word):
        return cls.disassemble_rv32im(word)


def decode_instruction_fields(word):
    return RV32IMDisassembler.decode_instruction_fields(word)


def disassemble_rv32im(word):
    return RV32IMDisassembler.disassemble_rv32im(word)


def disassemble(word):
    return RV32IMDisassembler.disassemble(word)
